...
// console.log( h + '.' + v + ": " + text );
		...
		setupAudioElement( container, h + '.' + v + '.0', slide.getAttribute( 'data-audio-src' ), text, slide.querySelector( ':not(.fragment) > video[data-audio-controls]' ) );
		var i = 0;
		var  fragments;
...
//console.debug( "Preload: " + nextAudio.id );
		...
		if ( audioFile != null ) {
			...
			var audioName = prefix + indices + '.0' + suffix;
			try {
				// check if audio file exists 
				var xhr = new XMLHttpRequest();
				xhr.open('HEAD', audioName, true);
	 			xhr.onload = function() {
