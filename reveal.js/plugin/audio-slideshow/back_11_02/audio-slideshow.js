...
(function() {
    ...
    /*var path;
    path = document.currentScript.src.slice(0, -42);*/
    ...
    Reveal.addEventListener('menuAudioSelected', function(event) {
        ...
        if (iconMode) {
            ...
            if (labelPos.x != undefined && iconPos.x != undefined) {
                setLocation(icon_id, labelPos, iconPos, false)
            }
        } else {
            ...
            if (labelPos.x != undefined && iconPos.x != undefined) {
                setLocation(label_id, labelPos, iconPos, true)
            }
        }
    ...
    function selectLabel(sub_indices) {
        ...
        if (labelPos.x != undefined && iconPos.x != undefined) {
            setLocation(id, labelPos, iconPos, true)
        }
    ...
    function selectIcon(sub_indices) {
        ...
        if (labelPos.x != undefined && iconPos.x != undefined) {
            setLocation(id, labelPos, iconPos, false)
        }
    }

    function setLocation(id, labelPos, iconPos, ifLabel) {
        var labelId;
        var iconId;
        if (ifLabel) {
            labelId = id;
            iconId = id.replace(/label-/, "icon-");
        } else {
            iconId = id;
            labelId = id.replace(/icon-/, "label-");
        }
        if (!paging) {
            var iconStyleString = "position:absolute; to p: 20px; left:" + iconPos.x + "px; top:" + iconPos.y + "px; width: 40px";
			//if(newWidth && newHeight)
			/*var labelStyleString = "position:absolute; to p: 20px; background-color : #C0C0C0 ; height: " + newHeight + "px; left:" + labelPos.x + "px; top:" + labelPos.y + "px; width :" + newWidth + "px;";*/
		    //else
            var labelStyleString = "position:absolute; to p: 20px; background-color : #C0C0C0 ; height: " + document.getElementById(labelId).offsetHeight + "px; left:" + labelPos.x + "px; top:" + labelPos.y + "px; width :" + document.getElementById(labelId).offsetWidth + "px;";
            document.getElementById(iconId).setAttribute('style', iconStyleString);
            document.getElementById(iconId).style.zIndex = "1000";
            document.getElementById(labelId).setAttribute('style', labelStyleString);
            document.getElementById(labelId).style.zIndex = "1000";
            document.querySelector(".reveal").appendChild(document.getElementById(labelId));
            document.querySelector(".reveal").appendChild(document.getElementById(iconId));
            if (ifLabel)
                document.getElementById(iconId).style.display = "none";
            else
                document.getElementById(labelId).style.display = "none";
        } else {
            subTitleLayout(document.getElementById(labelId), document.getElementById(iconId), ifLabel);
        }

    }

    function subTitleLayout(labelElement, iconElement, labelTest) {
        ...
        if (subTitleRegion == null) {
            ...
            subTitleRegion.setAttribute('style', "position: fixed;  z-index: 10; left:10%;right:10%;");
        }
        subTitleRegion.style.display = "block";
		document.body.appendChild(subTitleRegion);
		if(labelElement.offsetHeight != subTitleRegion.offsetWidth && labelElement.offsetWidth != subTitleRegion.offsetWidth){
				labelElement.heightMemo = labelElement.offsetHeight;
				labelElement.widthMemo = labelElement.offsetWidth;
                labelElement.childNodes[4].heightMemo = labelElement.childNodes[4].offsetHeight;
				labelElement.childNodes[4].widthMemo = labelElement.childNodes[4].offsetWidth;				
		}
				
        var labelStyleString = "position:fixed; font-size:25px ; font-family: 'Times New Roman'; width :" + subTitleRegion.offsetWidth + "px; height: " + (subTitleRegion.offsetHeight - subTitleRegion.offsetHeight*0.2) + "px;";
        var iconStyleString = "position:fixed; width: 40 px; font-size:35px ; ";
        labelElement.setAttribute('style', labelStyleString);
        labelElement.style.zIndex = "1000";
        iconElement.setAttribute('style', iconStyleString);
        iconElement.style.zIndex = "1000";
        subTitleRegion.appendChild(labelElement);
        subTitleRegion.appendChild(iconElement);
		labelElement.childNodes[4].style.height = (subTitleRegion.offsetHeight - subTitleRegion.offsetHeight*0.2 - 20) + "px";
		labelElement.childNodes[4].style.width = (subTitleRegion.offsetWidth - 20) + "px";
    ...
    function subTitlePosition(Px, Py, id) {
        var labelId;
        var iconId;
        labelPos.x = Px;
        labelPos.y = Py;
        ...
        if (Px < 0) {
            labelPos.x = 0;
            iconPos.x = 0;
        } else if (Px + document.getElementById(iconId).offsetWidth >= document.querySelector(".reveal").offsetWidth) {
            labelPos.x = document.querySelector(".reveal").offsetWidth - document.getElementById(labelId).offsetWidth;
            iconPos.x = document.querySelector(".reveal").offsetWidth - document.getElementById(iconId).offsetWidth;
        } 
		else if (Px + document.getElementById(labelId).offsetWidth >= document.querySelector(".reveal").offsetWidth) {
            labelPos.x = document.querySelector(".reveal").offsetWidth - document.getElementById(labelId).offsetWidth;
            iconPos.x = document.querySelector(".reveal").offsetWidth - document.getElementById(labelId).offsetWidth; 
        } 
		if (Py < 0) {
			labelPos.y = 0;
			iconPos.y = 0;
		} else if (Py + document.getElementById(iconId).offsetHeight >= document.querySelector(".reveal").offsetHeight* (1 - proportion) && !paging) {
			subTitleLayout(document.getElementById(labelId), document.getElementById(iconId), labelTest);
			paging = true;
			return
		} else if (Py + document.getElementById(labelId).offsetHeight >= document.querySelector(".reveal").offsetHeight* (1 - proportion) && !paging) {
			subTitleLayout(document.getElementById(labelId), document.getElementById(iconId), labelTest);
			paging = true;
			return
		}
		else if (Py + subTitleRegionWidth >= document.querySelector(".reveal").offsetHeight * (1 - proportion) && paging) {
		subTitleLayout(document.getElementById(labelId), document.getElementById(iconId), labelTest);
		paging = true;
		return;
		} 
		...
		var height = document.getElementById(labelId).offsetHeight;
	    var width = document.getElementById(labelId).offsetWidth;
		var contentHeight = document.getElementById(labelId).childNodes[4].offsetHeight;
		var contentWidth = document.getElementById(labelId).childNodes[4].offsetWidth;
	    if(document.getElementById(labelId).heightMemo && document.getElementById(labelId).widthMemo){
			height = document.getElementById(labelId).heightMemo;
		    width = document.getElementById(labelId).widthMemo;
		}
		if(document.getElementById(labelId).childNodes[4].heightMemo && document.getElementById(labelId).childNodes[4].widthMemo){
			contentHeight = document.getElementById(labelId).childNodes[4].heightMemo;
			contentWidth = document.getElementById(labelId).childNodes[4].widthMemo;
		}

        var labelStyleString = "position:absolute; to p: 20px; background-color : #C0C0C0 ; height:" + height + "px; left:" + labelPos.x + "px; top:" + labelPos.y + "px;width :" + width + "px;";
        var iconStyleString = "position:absolute; to p: 20px; width: 40px;left:" + iconPos.x + "px; top:" + iconPos.y + "px;";;
        document.getElementById(labelId).setAttribute('style', labelStyleString);
        document.getElementById(labelId).style.zIndex = "1000";
		document.getElementById(labelId).childNodes[4].style.height = contentHeight + "px";
		document.getElementById(labelId).childNodes[4].style.width = contentWidth + "px";
        ...
        if (paging) {
            document.querySelector(".reveal").appendChild(document.getElementById(labelId));
            document.querySelector(".reveal").appendChild(document.getElementById(iconId));
            document.getElementById("subTitleRegion").style.display = "none";
            paging = false;
        }
    ...
    function setupAudioElement(container, indices, audioFile, text, videoElement, subtitle) {
        ...
        for (var i = 0; i < textArray.length; i++) {
            ...
            labelElement.setAttribute('style', "position: absolute; to p: 20px; background-color : #C0C0C0 ; left:20%; right: 20%; bottom: 10px; height: "+ subTitleHeight +"em; width :" + subTitleWidth + "px;");
            labelElement.id = "label-" + indices + "." + i;
			labelElement.className = "subTitleContent";
            labelElement.style.display = "none";
            labelElement.style.zIndex = "1000";
            labelElement.draggable = true;
			labelElement.name = "subtitleDiv";
			
		    var leftUpResizer = document.createElement('div');
			leftUpResizer.setAttribute('style',"width: 10px; height: 10px; background: #C0C0C0; position:absolute; left: 0; top: 0; cursor: nw-resize; z-index = 1000");
			var rightUpResizer = document.createElement('div');
			rightUpResizer.setAttribute('style',"width: 10px; height: 10px; background: #C0C0C0; position:absolute; right: 0; top: 0; cursor: ne-resize; z-index = 1000");
			var leftDownResizer = document.createElement('div');
			leftDownResizer.setAttribute('style',"width: 10px; height: 10px; background: #C0C0C0; position:absolute; left: 0; bottom: 0; cursor: sw-resize; z-index = 1000");
			var rightDownResizer = document.createElement('div');
			rightDownResizer.setAttribute('style',"width: 10px; height: 10px; background: #C0C0C0; position:absolute; right: 0; bottom: 0; cursor: se-resize; z-index = 1000");
		    ...
		    leftUpResizer.onclick = function(e){
				e.stopPropagation();
			}
			...
			function removelu(e){
				...
				if (style.styleSheet) {
			     ...
			     resize = false;	
				 currentLabel.heightMemo = currentLabel.offsetHeight;
				 currentLabel.widthMemo  = currentLabel.offsetWidth;	
                 currentLabel.childNodes[4].heightMemo = currentLabel.childNodes[4].offsetHeight;
				 currentLabel.childNodes[4].widthMemo  = currentLabel.childNodes[4].offsetWidth;				 
			}
		    ...
		    rightUpResizer.onclick = function(e){
				e.stopPropagation();
			}
		    ...
		    leftDownResizer.onclick = function(e){
				e.stopPropagation();
			}
		    ...
		    rightDownResizer.onclick = function(e){
				e.stopPropagation();
			}
			...
			labelContent.setAttribute('style',"position: absolute ; z-index = 1000; left : 10px; top : 10px; overflow: auto; height: "+ (subTitleHeight - 0.6) +"em; width :" + (subTitleWidth - 12) + "px;");
            ...
            labelElement.addEventListener('dragstart', function(event) {
				if(!resize){
						...
						dataTransfer.x = document.getElementById(id).offsetLeft;
						dataTransfer.y = document.getElementById(id).offsetTop;
						var e = event || window.event;
						dataTransfer.MouseX = e.screenX;
						dataTransfer.MouseY = e.screenY;					
				}
            });
            ...
            labelElement.addEventListener('dragend', function(event) {
				if(!resize){
						...
						var e = event || window.event;
						Px = dataTransfer.x - (dataTransfer.MouseX - e.screenX);
						Py = dataTransfer.y - (dataTransfer.MouseY - e.screenY);
            ...
            icon.setAttribute('style', "position:absolute; to p: 20px; left:20%; width: 40px ; bottom: 30px;");
            ...
            icon.addEventListener('dragstart', function(event) {
                var dataTransfer = event.dataTransfer;
                dataTransfer.effectAllowed = "all";
                dataTransfer.x = document.getElementById(event.target.id).offsetLeft;
                dataTransfer.y = document.getElementById(event.target.id).offsetTop;
                var e = event || window.event;
                dataTransfer.MouseX = e.screenX;
                dataTransfer.MouseY = e.screenY;
            });
            ...
            icon.addEventListener('dragend', function(event) {
                ...
                Px = dataTransfer.x - (dataTransfer.MouseX - e.screenX);
                Py = dataTransfer.y - (dataTransfer.MouseY - e.screenY);
                subTitlePosition(Px, Py, event.target.id);
            });
            ...
            audioElement.addEventListener('play', (function(event, i, name,text) {
                return function(event) {
            ...
            })(event, i, indices,textArray[i]));

			/*var downloadButton = document.getElementsByClassName("downloadButton")[0];
			downloadButton.addEventListener('click', (function(event, i, name,text) {
                return function(event) {
	...
	jpInterface.id = "jp_container_1";
	jpInterface.className = "jp-audio";
	jpInterface.setAttribute('style', "position: relative; left: 8%; bottom:5px;z-index:500;");
    jpInterface.classList.remove("reveal");
	
	var jpInterface1 = document.createElement('div');
	jpInterface1.className = "jp-type-single";
    jpInterface1.classList.remove("reveal");
	
	var jpInterface2 = document.createElement('div');
	jpInterface2.className = "jp-gui jp-interface";
    jpInterface2.classList.remove("reveal");*/
	...
	downloadButton.className = "fa fa-download downloadButton";
	downloadButton.setAttribute('aria-hidden', "true");
	downloadButton.setAttribute('style', "font-size:15px");
	downloadButton.addEventListener('click', function(event) {
		iconId = currentIcon.id;
		labelId = iconId.replace("icon","label");
		label = document.getElementById(labelId);
		
		name = labelId.replace("label-","")+".ogg";
		text = label.childNodes[4].innerText;
		console.debug(textToSpeechURL + encodeURIComponent(text));
		if (textToSpeechURL != null && text != null) {
			var content = textToSpeechURL + encodeURIComponent(text);
	...
	volumeControl.appendChild(downloadButton);
	audioDiv.appendChild(volumeControl);
    /*jpInterface2.appendChild(volumeControl);
	jpInterface1.appendChild(jpInterface2);
	jpInterface.appendChild(jpInterface1);*/
	
    document.querySelector(".reveal").appendChild(audioDiv);
	//document.querySelector(".reveal").appendChild(jpInterface);
