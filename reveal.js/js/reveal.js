...
(function( root, factory ) {
	...
	var SLIDES_SELECTOR = '.slides section',
		...
		config = {
			...
			// Flags if subtitle should be visible when printing PDF
			showSubTitles: true,
		...
		toArray( dom.wrapper.querySelectorAll( SLIDES_SELECTOR ) ).forEach( function( slide ) {
			...
			if( slide.classList.contains( 'stack' ) === false ) {
				...
				if( config.showSubTitles ) {
					var subTitles = getSlideSubTitles( slide );
					if( subTitles ) {
						var subTitlesSpacing = 8;
						var subTitlesElement = document.createElement( 'div' );
						subTitlesElement.classList.add( 'subtitle-notes' );
						subTitlesElement.classList.add( 'subtitle-notes-pdf' );
						subTitlesElement.innerHTML = subTitles;
						subTitlesElement.style.left = ( subTitlesSpacing - left ) + 'px';
						//subTitlesElement.style.bottom = ( subTitlesSpacing - top ) + 'px';
						subTitlesElement.style.width = ( pageWidth - subTitlesSpacing*2 ) + 'px';
						//subTitlesElement.style.left = '0px';
						subTitlesElement.style.bottom = '-25px';
						//subTitlesElement.style.width = pageWidth + 'px';
						slide.appendChild( subTitlesElement );
					}
			    }
				}
	 ...
	 */
	function slide( h, v, f, o, subTitle ) {
		...
		if( typeof f !== 'undefined' ) {
			navigateFragment( f, undefined, subTitle );
		}
	...
	*/
	...
	function getSlideSubTitles( slide ) {
		...
		if( slide.hasAttribute( 'data-audio-text' ) ) {
			return slide.getAttribute( 'data-audio-text' ).split('.')[0];
		}
	...
	function selectAudio(h, v, j ,k) {

		dispatchEvent( 'menuAudioSelected', {
					'indexh': h,
					'indexv': v,
					'indexf': j,
					'indexs' : k
	...
	function downloadAudio() {
		dispatchEvent( 'downloadAudio', {
				
		} );
	}
	
	function timerSwitchMode(show) {
		dispatchEvent( 'timerSwitchMode', {
				'shown' : show
		} );
	}
	
	function recordSwitchMode(show) {
		dispatchEvent( 'recordSwitchMode', {
				'shown' : show
		} );
	}
	 ...
	 */
	function navigateFragment( index, offset, subTitle) {
		...
		if( currentSlide && config.fragments ) {
			...
			if( fragments.length ) {
				...
				if( fragmentsHidden.length ) {
					dispatchEvent( 'fragmenthidden', { fragment: fragmentsHidden[0], fragments: fragmentsHidden, subTitle: subTitle } );
				}

				if( fragmentsShown.length ) {
					dispatchEvent( 'fragmentshown', { fragment: fragmentsShown[0], fragments: fragmentsShown, subTitle: subTitle } );
				}
